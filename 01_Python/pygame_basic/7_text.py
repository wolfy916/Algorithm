...
while running:
    dt = clock.tick(60) # 게임화면의 초당 프레임수를 설정, fps 10 일 경우 0.1초에 한번 동작

    for event in pygame.event.get(): # 어떤 이벤트가 발생하였는가?
        if event.type == pygame.QUIT: # 창이 닫히는 이벤트가 발생 하였는가?
            running = False # 게임이 진행중이 아님
        
        if event.type == pygame.KEYDOWN: # 키가 입력되었는지 확인
    ...
    # 충돌 체크
    if character_rect.colliderect(enemy_rect): # 캐릭터와 enemy의 충돌 여부 확인
        print("충돌했어요")
        running = False
