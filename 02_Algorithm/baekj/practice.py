...
def bfs(s_i, s_j):  # s_i, s_j: 아기 상어 현재 위치
    ...
    food = 0
    while q:
        i, j, k = q.pop(0)  # i, j: 방문한 곳의 좌표, k: 방문한 곳까지의 거리, s: 방문한 곳의 물고기 사이즈

        # 0 < 물고기 사이즈 < 상어 사이즈를 만족하면 리턴
        if 0 < k < shark_size:
...
time = 0  # 시간 초기값
while fish_n[:shark_size].count(0) < shark_size:
    # for i in range(N+2):
    #     print(*area[i])
    # else:
